# -*- coding: utf-8 -*-
"""
Created on Sun Jan 24 19:26:39 2021
...
def snort_start():
    path_config = d+"\\Snort\\etc\\snort.conf"
    if os.path.isfile(path_config):
        print("snort.conf found")
    else:
        print("Creating snort.conf")
        create_config()
    ...
    try:
        print("Snort started.")
        engine = d +"\\Snort\\bin\\snort.exe"
        config = d + "\\Snort\\etc\\snort.conf"
        command = engine + " -i 1 -c " + config + " -A full"
        process = subprocess.Popen(command,stdout=subprocess.PIPE, shell=True)
...
def snort_kill(**kwargs):
    ...
    command_kill = "taskkill /im snort.exe /F"
    kill = subprocess.Popen(command_kill, stdout=subprocess.PIPE)
...
def print_time():
    ...
    for i in range(time_remain):
        clear()
        print ("Time remian", time_remain, " sec(s)")
        time.sleep(1)
    clear()
    print("Done.")
...
def create_config():
    try:
        path = d +"\\Snort"
        path_config = path+"\\etc\\snort.conf"
        config = f"""#--------------------------------------------------
        ...
        var RULE_PATH {path}\\etc\\community-rules
        # var SO_RULE_PATH C:\\Snort\\so_rules
        var PREPROC_RULE_PATH {path}\\preproc_rules
        ...
        var WHITE_LIST_PATH {path}\\rules
        var BLACK_LIST_PATH {path}\\rules
        ...
        dynamicpreprocessor directory {path}\\lib\\snort_dynamicpreprocessor
        
        # path to base preprocessor engine
        dynamicengine {path}\\lib\\snort_dynamicengine\\sf_engine.dll
        
        # path to dynamic rules libraries
        # dynamicdetection directory {path}\\lib\\snort_dynamicrules
        ...
        """ 
        with open(path_config, "w") as out:
            out.write(config)      
        out.close()
